...
type matcher = acceptor -> fragment -> fragment option

type rev_matcher matcher a b c = matcher b a c
...
let rec match_star  reverse_matcher frag accept =
  match accept frag with
    | None ->
	 reverse_matcher frag
		(fun frag1 ->
		   if frag == frag1
		   then None
		   else match_star  reverse_matcher frag1 accept)
    | ok -> ok
