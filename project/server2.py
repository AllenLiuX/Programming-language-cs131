...
api_key = 'AIzaSyAI4IRkMeECJEBQCKH6wrRiWu1X4k76xII'
...
async def whatsat(client_name, radius, num_results, cmd_time, cmd, writer):
    ...
    lat, lon, float(radius), api_key)

    # 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=34.068930.-118.445127&radius=10&key=AIzaSyC0JifQV9GO11J10B3wzO6mATabaAwGaiIAIzaSyC0JifQV9GO11J10B3wzO6mATabaAwGaiI'
...
async def flood(client_name):
    ...
    at_cmd = 'AT %s %s %s %s %s %s' % (
    client_name, client['server_name'], client['latitude'], client['longitude'], str(client['time_diff']),
    str(client['cmd_time']))
